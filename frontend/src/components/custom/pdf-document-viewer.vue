...
async function loadDocument(url: string) {
  ...
  try {
    ...
    // 先用 fetch 把整个 PDF 下下来再喂给 pdf.js。
    // 原因：直接把 url 交给 pdf.js 时，它会发 HEAD/Range 探测以决定是否用分块加载；
    // 但 MinIO 的预签名 URL 只对 GET 方法签名，HEAD/Range 会 SignatureDoesNotMatch → 加载失败。
    // 走 fetch 就只发一次签过名的 GET，最稳。
    const fetchHeaders: Record<string, string> = {};
    if (shouldAttachAuthHeaders && authorization) {
    ...
    if (!response.ok) {
      throw new Error(`fetch PDF failed: HTTP ${response.status}`);
    }
    ...
    renderError.value = `PDF 加载失败：${detail}`;
  } finally {
    if (currentToken === lifecycleToken) {
      documentLoading.value = false;
    }
